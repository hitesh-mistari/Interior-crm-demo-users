...
router.delete("/payments/:id", async (req, res) => {
  ...
  try {
    const result = await withTransaction(async (client: any) => {
      ...
      await client.query(
        `INSERT INTO payment_trash (original_id, snapshot_json, deleted_at, deleted_by, reason, retention_until)
...
// RESTORE
// ==========================
router.post("/payments/:id/restore", async (req, res) => {
  const id = req.params.id;
  const actorUserId = req.body.actorUserId ?? null;
  ...
  try {
    const result = await withTransaction(async (client: any) => {
      ...
      await client.query(`DELETE FROM payment_trash WHERE original_id = $1`, [id]);

      await client.query(
        `INSERT INTO trash_logs (item_type, item_id, action, actor_user_id)
         VALUES ('payment', $1, 'restore', $2)`,
        [id, actorUserId]
      );
    ...
    console.error("RESTORE payments error:", err);
    res.status(400).json({ error: err.message });
  }
});
...
// TRASH LIST
// ==========================
router.get("/trash/payments", async (_req, res) => {
  try {
    const rows = await query(
      `SELECT * FROM payment_trash ORDER BY deleted_at DESC`
    );
    res.json(rows);
  } catch (err) {
    console.error("GET trash/payments error:", err);
...
// ==========================
router.delete("/trash/payments/:id", async (req, res) => {
  const id = req.params.id;
  const actorUserId = req.body.actorUserId ?? null;
  ...
  try {
    ...
    console.error("PURGE payments error:", err);
    res.status(400).json({ error: err.message });
  }
});
